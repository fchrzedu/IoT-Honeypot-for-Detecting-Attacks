...
# Original location: /IoT-Honeypot-for-Detecting-Attacks/Honeypot Project/vanilla-honeypot/cowrie/src/cowrie/shell/

"""
Modified: HoneyPotShell:
"""
...
class HoneyPotShell:
    def __init__(
        ...
        # ========== DEVICE PROFILE INJECTION
        try:
            # Traverse cowrie object model
            # protocol > terminal > transport > session > device_profile
            session = ( # we return None as this is during initialisation, we are only checking whether it is there
                self.protocol.terminal.transport.session
                if hasattr(self.protocol, "terminal")
                else None
            )
            if session and hasattr(session, "device_profile"):
                profile = session.device_profile

                # Override hostname if defined in profile
                if "hostname" in profile:
                    self.protocol.hostname = profile["hostname"]

                # Store shell prompt if provided
                if "shell_prompt" in profile:
    ...
    def showPrompt(self) -> None:
        ...
        if CowrieConfig.has_option("honeypot", "prompt"):
            ...
            # =====================================================
            # Device-profile aware shell prompt
            # =====================================================

            # If device profile defines a fixed shell prompt, use it
            if hasattr(self.protocol, "device_shell_prompt"):
                prompt = self.protocol.device_shell_prompt
            else:
