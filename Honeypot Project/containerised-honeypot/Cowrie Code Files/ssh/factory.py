    ...
    def startFactory(self) -> None:
        ...
        for _moduli in _modulis:
            try:
                ...
                pass
            # ========== DEVICE PROFILE SSH BANNER ==========
            # Add cowrie as root to python path
            ...
            try:
                ...
                # Select random SSH banner from device profiles
                random_profile = random.choice(list(DEVICE_PROFILES.values()))
                self.ourVersionString = random_profile['ssh_banner'].encode('utf-8')
                log.msg(f"[DEVICE PROFILES] Using SSH banner: {random_profile['ssh_banner']}")

            except Exception as e:

                # If device_profiles.py fails, use default from config
                log.msg(f"[WARNING] Failed to load device profile banner: {e}")
